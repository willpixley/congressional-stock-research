...
def create_segments():
    ...
    for member_id, tlist in member_trades.items():
        ...
        for seg in open_segments_by_member.get(member_id, []):
            ...
            matching_sell = next(
                (s for s in candidates if s.date > buy.date and s.id not in used_sells),
                None,
            )
    ...
    with transaction.atomic():
        if segments_to_create:
            TradeSegment.objects.bulk_create(segments_to_create, ignore_conflicts=True)
    ...
    print(f"Created {total} new segments")
